const CACHE_NAME = 'car-trip-pwa-v9-compact-top-buttons';
const ASSETS = ['./','./index.html','./manifest.json','./icon-192.png','./icon-512.png'];

function compactHtml(html){
  const patch = `
<style id="compact-top-buttons">
@media(max-width:760px){
  .app{padding:6px!important;height:calc(100dvh - var(--navH))!important}
  .grid{grid-template-rows:.62fr 1.38fr!important;gap:6px!important}
  ...
  .side{grid-template-rows:1fr .43fr!important;gap:6px!important}
  ...
  .actions{grid-template-columns:repeat(2,1fr)!important;gap:6px!important;margin-top:6px!important}
  button{padding:9px 6px!important;font-size:11px!important;min-height:42px!important}
...
@media(max-height:720px){
  ...
  return html.includes('compact-top-buttons') ? html : html.replace('</head>', patch + '</head>');
}
